...
TE_RF.1 <- toc(log = TRUE)

tic()
M5.RF.2 <- Evaluacion1(Metodo = "RF.2",
                       workers = availableCores())
TE_RF.2 <- toc(log = TRUE)
...
G.TE_RF.1 <- data.frame(TE = TE_RF.1[["callback_msg"]]) %>% 
  mutate(Modelo = "Random Forest",
         Nombre = "RF 1")

G.TE_RF.2 <- data.frame(TE = TE_RF.2[["callback_msg"]]) %>% 
  mutate(Modelo = "Random Forest",
         Nombre = "RF 2")

TE_M5 <- bind_rows(G.TE_RF.1, G.TE_RF.2) %>% 
  mutate(Modelo = as.factor(Modelo),
         Nombre = as.factor(Nombre))
